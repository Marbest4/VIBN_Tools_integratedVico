...
{
    public class FeeReadingUnit : FeeAbstractObject, IPlausibilityCheck
    {
        ...
        public FeeReadingUnit()
        {
            Guid = Guid.NewGuid();
            FeeType = nameof(ReadingUnitUdt);
            Visible = true;
        ...
        public override async Task<bool> CreateAsync()
        {
            ...
            await Services.ApiInstance.Object.SetPropertyAsync(Guid, nameof(UDTComponent.ReadWriteTypes), UdtDefinition, "UDT");
        ...
        public async Task CheckObjectIssuesAsync(IEnumerable<FeeAbstractObject> newObjects)
        {
            ...
            Guid guid;
            var doReadConnected = Slots.TryGetValue("DoRead", out guid) && guid != Guid.Empty;
            ...
            if (!doReadConnected)
                PlausibilityIssues.Add(new PlausibilityIssue($"Slot 'DoRead' nicht verknüpft", Severity.Error));
